...
void AddIcoEdgePoints(
  ...
  AddSingleEdge(inds_s[0], inds_s[1], num_additional, edge_data, vertices, v_ind);
  AddSingleEdge(inds_s[1], inds_s[2], num_additional, edge_data, vertices, v_ind);
  AddSingleEdge(inds_s[0], inds_s[2], num_additional, edge_data, vertices, v_ind);
}

void FillIcoFace(
    const std::vector<size_t>& inds,
    const std::map<std::pair<size_t, size_t>, std::vector<std::pair<size_t, std::array<double, 3>>>>& edge_data,
    std::vector<std::array<double, 3>>* vertices, size_t* v_ind, std::vector<std::vector<size_t>>* faces,
    size_t* face_ind) {
  ...
  for (size_t i = 1; i < left_side.size(); i++) {
    ...
    if (permutation_sign) {
      (*faces)[*face_ind] = {bottom_row[0].first, left_side[i].first, bottom_row[1].first};
    } else {
      (*faces)[*face_ind] = {bottom_row[0].first, bottom_row[1].first, left_side[i].first};
    }
    (*face_ind)++;
    for (size_t j = 1; j < bottom_row.size() - 1; j++) {
      ...
      if (permutation_sign) {
        (*faces)[(*face_ind)++] = {ind_top_left, ind_top_right, ind_bottom_left};
        (*faces)[(*face_ind)++] = {ind_bottom_left, ind_top_right, ind_bottom_right};
      } else {
        (*faces)[(*face_ind)++] = {ind_top_left, ind_bottom_left, ind_top_right};
        (*faces)[(*face_ind)++] = {ind_bottom_left, ind_bottom_right, ind_top_right};
      }
...
std::pair<std::vector<std::array<double, 3>>, std::vector<std::vector<size_t>>> FastIcoSphere(const int num_additional,
  ...
  size_t num_vertices = 12 + 30 * num_additional + 20 * NumInternalVertices(num_additional);
  ...
  for (size_t i = 0; i < faceIndices.size(); i++) {
    ...
    // points.
    AddIcoEdgePoints(num_additional, faceIndices[i], &skeleton_points, &vertexPositions, &vertex_counter);
    // 3) Add the faces and rest of "internal" vertices, since the needed vertices have been defined
    FillIcoFace(faceIndices[i], skeleton_points, &vertexPositions, &vertex_counter, &fineFaceIndices, &face_counter);
  }
